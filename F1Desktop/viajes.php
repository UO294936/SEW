	...
	class Carrusel {
		...
		public function crearCarrousel(){
			...
			for($i=0;$i<$this->numeroFotos;$i++) {
				...
				$titulo = htmlspecialchars($json->items[$i]->title, ENT_QUOTES, 'UTF-8');
				$URLfoto = $json->items[$i]->media->m;  

				$carrouselHTML .=  "<img alt='Imagen de {$titulo}' src='{$URLfoto}' />";
			}

			$carrouselHTML .= "<button aria-label='Siguiente'> &gt; </button>";
			$carrouselHTML .= "<button aria-label='Anterior'> &lt; </button>";
			$carrouselHTML .= "</article>";
...
<head>
	...
	<!-- El validador HTML genera algunas advertencias, provocados por el código HTML inyectado por la API de google maps -->
</head>
...
<body>
	...
	<main>
		...
		<button>Generar</button>
		<div hidden></div>
		<!-- El validador HTML genera numerosas advertencias y errores, provocados por el código HTML inyectado por la API de google maps al generar el mapa dinámico -->
		<!-- La herramienta de accesibilidad AChecker detecta un problema de contraste entre el color de texto y fondo del mapa dinámico de Google Maps -->
